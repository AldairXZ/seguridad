...
export class VistaGrupoComponent implements OnInit {
    ...
    this.http.get<any>('http://localhost:3000/api/groups', { headers: this.getHeaders() }).subscribe({
      next: (res) => {
        ...
        if (grupo && grupo.empleados) {
          this.empleadosDelGrupo = grupo.empleados;
        } else {
          this.empleadosDelGrupo = [];
        }
      }
    });
  }
  ...
  guardarTicket() {
    if (this.ticketActual.titulo && this.ticketActual.estadoActual && this.ticketActual.prioridad && this.ticketActual.groupId && this.ticketActual.fechaLimite) {
      ...
      this.messageService.add({ severity: 'warn', summary: 'Aviso', detail: 'Faltan campos obligatorios o contexto de grupo, incluyendo la Fecha Límite.' });
    }
  }
