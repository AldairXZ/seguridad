...
pool.query('ALTER TABLE groups ADD COLUMN IF NOT EXISTS empleados jsonb DEFAULT \'[]\'::jsonb').catch(() => {});
...
app.post('/api/groups', validateGroup, async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `INSERT INTO groups (nombre, categoria, nivel, autor, miembros, tickets, empleados) 
       VALUES ($1, $2, $3, $4, 1, 0, $5) RETURNING *`,
      [req.body.nombre, req.body.categoria, req.body.nivel, autor, JSON.stringify(empleados)]
    );
...
app.put('/api/groups/:id', validateGroup, async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `UPDATE groups SET nombre = $1, categoria = $2, nivel = $3, empleados = $4 WHERE id = $5 RETURNING *`,
      [req.body.nombre, req.body.categoria, req.body.nivel, JSON.stringify(empleados), id]
    );
